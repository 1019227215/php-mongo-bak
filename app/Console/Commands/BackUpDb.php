...
class BackUpDb extends Command
{
    ...
    private function proData($orghost, $trghost)
    {
        ...
        $exppid = "grep 'mongodump -h {$orghost['host']}'|grep '{$orghost['port']}'|grep '{$orghost['tables']}'";
        $imppid = "grep 'mongorestore -h {$trghost['host']}'grep '{$trghost['port']}'|grep '{$orghost['tables']}'";

        if (self::getProsize($exppid) > 2 || self::getProsize($imppid) > 2) {
            return "别急，前面的还没跑完！";
        }

        $expcmd = "{$this->sudo}{$this->mongodir}mongodump -h {$orghost['host']} --port {$orghost['port']} -u {$orghost['username']} -p '{$orghost['password']}' --authenticationDatabase admin -d {$orghost['database']} {$table} {$whe} -o {$this->mongobak}";
